    ...
    @if($carrello_lista != null)
                                ...
                                <tr class="tr-carrello">
                                    <td class="col-xs-2">
                                        <a href="/catalogo/prodotto/{!!$carrello->prodotto_id!!}">
                                            <img class="thumbnail"
                                                 src="{!!$carrello->immagine_url.'/'.$carrello->immagine_nome!!}"
                                                 height="100"/>
                                        </a>
                                    </td>
                                    ...
                                    <td class="cart-col-qta col-xs-1">
                                        {!! Form::input('number','quantita',
                                        $carrello->quantita,array('class'=>'form-control input-small
                                        quantita','data-token' => csrf_token()
                                        ,'data-id'=>url('/carrello/'.$carrello->id),'data-token'=>csrf_token()))
                                        !!}
                                    </td>
                                    <td class="cart-col-price col-xs-1">
                                        {!!number_format((float)$carrello->prezzo*$carrello->quantita, 2, '.',
                                        '')!!} {!! $valuta->simbolo !!}
                                    </td>
                                    <td class="cart-col-delete col-xs-1">
                                        <a href="{!!url('/carrello/'.$carrello->id)!!}"
                                           data-id="{!!$carrello->id!!}" data-token="<?= csrf_token() ?>"
                                           class="btn btn-danger btn-large btn-del-item-cart">{!!Lang::choice('messages.pulsante_elimina',0)!!}</a>
                                    </td>
                                </tr>
                                <?php
        ...
        <div class="row">
            <div class="panel panel-default">
                <div class="panel-body">
                    <div class="col-xs-5">
                        <h4 class="cart-tot-price">{!!Lang::choice('messages.totale_provvisorio',0)!!} (
                            {!!$index!!}@if($index==1) {!!Lang::choice('messages.articolo',0)!!} @else
