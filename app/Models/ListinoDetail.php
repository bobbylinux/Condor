        ...
        switch ($type) {
            case "like":
                ...
                $result = DB::table('prodotti')
                        ...
                        ->join('listini_master', 'listini_detail.listino', '=', 'listini_master.id')
                        ->join('immagini_prodotti','prodotti.id','=','immagini_prodotti.prodotto')
                        ->join('immagini', 'immagini.id', '=', 'immagini_prodotti.immagine')
                        ->where('prodotti.cancellato', '=', false)
                        ->where('listini_detail.cancellato', '=', false)
                        ->where('listini_master.cancellato', '=', false)
...
//                        ->where('listini_detail.id', '=', $id_detail)
    ...
    public function searchById($id_detail) {
        $result = DB::table('prodotti')
                ...
                ->join('listini_master', 'listini_detail.listino', '=', 'listini_master.id')
                ->join('immagini_prodotti','prodotti.id','=','immagini_prodotti.prodotto')
                ->join('immagini', 'immagini_prodotti.immagine', '=', 'immagini.id')
                ->where('prodotti.cancellato', '=', false)
                ->where('listini_detail.cancellato', '=', false)
                ->where('listini_master.cancellato', '=', false)
                ->where('immagini.cancellato', '=', false)
    ...
    public function getProductImages($id_detail) {
        $result = DB::table('immagini')
                ->join('immagini_prodotti','immagini.id','=','immagini_prodotti.immagine')
                ->join('prodotti','prodotti.id','=','immagini_prodotti.prodotto')
                ->join('listini_detail','listini_detail.prodotto','=','prodotti.id')
                ->where('listini_detail.cancellato','=',false)
                ->where('prodotti.cancellato','=',false)
                ->where('immagini.cancellato','=',false)
